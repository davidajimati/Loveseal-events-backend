...
async function verifyToken(res: Response, token: string) {
    try {
        ...
        if (err instanceof Prisma.PrismaClientKnownRequestError) {
            if (err.code === "P2021") {
                return response.internalServerError(res, "Invalid or Expired token. Please try again");
            }
        }
        return response.internalServerError(res, "something went wrong");
    }
...
async function verifyOtp(res: Response, validationPayload: otpValidationType) {
    try {
        ...
        if (err instanceof Prisma.PrismaClientKnownRequestError) {
            if (err.code === "P2021") {
                return response.internalServerError(res, "Operation failed. Contact admin");
            }
        }
        return response.internalServerError(res, "something went wrong");
    }
