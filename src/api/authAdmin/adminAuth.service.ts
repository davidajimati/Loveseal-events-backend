  ...
  try {
    ...
    if (!adminUser) {
      return response.badRequest(
        res,
        "Sorry, this action is reserved only for admin users.",
      );
    }
    ...
    try {
      token = jwt.sign(
        { id: adminUser.adminUserId, email: adminUser.email },
        SECRET,
        { expiresIn: "1d" },
      );
    } catch (error) {
    ...
    const { dateCreated, updatedAt, paymentRecords, ...necessaryDetails } =
      adminUser;
...
async function verifyToken(res: Response, token: string) {
  try {
    ...
    adminUser = await prisma.adminUserRecords.findUnique({
      where: { adminUserId: decoded.id },
    });
    const { dateCreated, updatedAt, paymentRecords, ...necessaryDetails } =
      adminUser;
    return response.successResponse(res, {
...
export { verifyToken, generateToken, generateOtp, verifyOtp };
